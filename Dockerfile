...
RUN CGO_ENABLED=0 GOOS=$TARGETOS GOARCH=$TARGETARCH \
    go build -trimpath -ldflags="-s -w -X main.version=${VERSION}" -o /out/rightsizer ./cmd/rightsizer \
 && mkdir -p /out/data
...
ENV RIGHTSIZER_DATA=/data
VOLUME /data
EXPOSE 8443
USER 65532:65532
HEALTHCHECK --interval=1m --timeout=5s CMD ["/usr/local/bin/rightsizer", "status"]
ENTRYPOINT ["/usr/local/bin/rightsizer"]
CMD ["daemon"]
