...
func demo() *analysis.Result {
	...
	for c, name := range []string{"prod-cl01", "prod-cl02", "dmz-cl01"} {
		for h := 0; h < 4+2*(1-c/2); h++ {
			inv.Hosts = append(inv.Hosts, vc.Host{Ref: fmt.Sprintf("host-%d-%d", c, h), Name: fmt.Sprintf("esx%02d.%s", h+1, name), Cluster: name,
				CPUModel: "Intel(R) Xeon(R) Gold 6338 CPU @ 2.00GHz", Sockets: 2, Cores: 64, MHz: 2000, MemBytes: 1024 << 30, Connected: true})
		}
		for v := 0; v < 25; v++ {
	...
	r := analysis.Analyze(inv, st, analysis.ProfileByName("balanced"), start, end, 14*24*time.Hour)
	r.Final = true
